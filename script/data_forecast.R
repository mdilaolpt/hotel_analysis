...
occupancy_forecast <- function(df,
                           forecast_horizon){
  ...
  # Plot actual and predicted values
  occupancy_plot <- ggplot(combined_data, aes(x = ds)) +
    geom_line(aes(y = y, color = "Actual")) +
    geom_line(aes(y = yhat, color = "Predicted")) +
    labs(x = "Date", y = "Values", color = "Data Type") +
    ggtitle(glue::glue("{hotel_name} Hotel Actual vs Predicted Occupancy Values for {forecast_horizon} days"))
...
## Occupancy-------------------------------------------
ICWAS_occ_forecast <-  occupancy_forecast(icwas_df,
                                  forecast_horizon = 90)
ggsave("icwas_occ_forecast_3mos.png", plot = ICWAS_occ_forecast)

# 1 year
ICWAS_occ_forecast <-  occupancy_forecast(icwas_df,
                                  forecast_horizon = 360)
ggsave("icwas_occ_forecast_1yr.png", plot = ICWAS_occ_forecast)
...
## Occupancy-------------------------------------------
EXIUS_occ_forecast <-  occupancy_forecast(exius_df,
                                          forecast_horizon = 90)
ggsave("exius_occ_forecast_3mos.png", plot = EXIUS_occ_forecast)

# 1 year
EXIUS_occ_forecast <-  occupancy_forecast(exius_df,
                                          forecast_horizon = 360)
ggsave("exius_occ_forecast_1yr.png", plot = EXIUS_occ_forecast)
...
#Occupancy --------------------------
GCBJN_occ_forecast <-  occupancy_forecast(gcbjn_df,
                                          forecast_horizon = 90)
ggsave("gcbjn_occ_forecast_3mos.png", plot = GCBJN_occ_forecast)

# 1 year
GCBJN_occ_forecast <-  occupancy_forecast(gcbjn_df,
                                          forecast_horizon = 360)
ggsave("gcbjn_occ_forecast_1yr.png", plot = GCBJN_occ_forecast)
...
#Occupancy --------------------------
phega_occ_forecast <-  occupancy_forecast(phega_df,
                                          forecast_horizon = 90)
ggsave("phega_occ_forecast_3mos.png", plot = phega_occ_forecast)

# 1 year
phega_occ_forecast <-  occupancy_forecast(phega_df,
                                          forecast_horizon = 360)
ggsave("phega_occ_forecast_1yr.png", plot = phega_occ_forecast)
...
#Occupancy --------------------------
ukpil_occ_forecast <-  occupancy_forecast(ukpil_df,
                                          forecast_horizon = 90)
ggsave("ukpil_occ_forecast_3mos.png", plot = ukpil_occ_forecast)

# 1 year
ukpil_occ_forecast <-  occupancy_forecast(ukpil_df,
                                          forecast_horizon = 360)
ggsave("ukpil_occ_forecast_1yr.png", plot = ukpil_occ_forecast)


#return wd to main folder
setwd("~/hotel_performance_analysis/") 
